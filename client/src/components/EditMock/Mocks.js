...
function Mocks() {
        ...
        async function getMock() {
            ...
            try {
                ...
                form.setFieldsValue({
                    ...
                    httpHeader: (data.data.httpHeader ? JSON.stringify(data.data.httpHeader) : undefined),
                    httpResBody: (data.data.resContentType == 'application/json') ? JSON.stringify(data.data.httpResBody) : data.data.httpResBody.content,
                })
    ...
    async function onFinish(values) {
        ...
        try {
            setSubmitting(true)
            const res = await axios.patch(`mocky/edit/${params.id}`, values)
            message.success("Your mocky API edited!")
            navigate('/my-mocks');
        } catch (error) {
            message.error(error.response.data.message);
        } finally {
        ...
        <>
            ...
            >
                ...
                <Form.Item
                    ...
                    name="httpResBody"
                >
                    <TextArea rows={11} typeof="object"></TextArea>
